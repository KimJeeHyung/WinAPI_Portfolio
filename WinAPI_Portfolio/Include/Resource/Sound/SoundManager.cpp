...
CSoundManager::~CSoundManager()
{
	...
	std::unordered_map<std::string, FMOD::ChannelGroup*>::iterator iter;
	std::unordered_map<std::string, FMOD::ChannelGroup*>::iterator iterEnd;

	iter = m_mapChannelGroup.begin();
	iterEnd = m_mapChannelGroup.end();
...
{
	// FMOD_RESULT : FMOD에서 제공하는 타입으로 FMOD의 기능을 사용할 때
	// 함수가 성공했는지 실패했는지를 판단하기 위한 값의 타입이다.
	...
	CreateSoundChannel("HitEffect");
	CreateSoundChannel("SE");
	CreateSoundChannel("Voice");
	CreateSoundChannel("UI");
	CreateSoundChannel("Announcer");
...
{
	// 매 프레임 시스템을 업데이트해야 사운드가 정상적으로 출력된다.
	m_System->update();
}
...
{
	std::unordered_map<std::string, FMOD::ChannelGroup*>::iterator iter;
	iter = m_mapChannelGroup.find(Name);
