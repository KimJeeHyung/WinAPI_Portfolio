...
{
	// 텍스처 파일이 있는 폴더의 경로를 얻어온다.
	const PathInfo* Path = CPathManager::GetInst()->FindPath(PathName);

	TCHAR FullPath[MAX_PATH] = {};

	// 경로가 있다면 FullPath에 복사한다.
	if (Path)
		lstrcpy(FullPath, Path->Path);

	// FullPath뒤에 텍스처 파일 이름을 붙여넣는다.
	...
	const std::string& PathName)
{
	...
	// resize : 미리 인자 개수만큼 push해놓는 함수이다.
	vecFullPath.resize(Size);

	// reserve : 미리 배열 공간을 확보하여 꽉 찼을 때 공간 재할당이 일어나는 것을
	// 방지해주는 역할을 할 때 사용한다.
	//vecFullPath.reserve(Size);

	for (size_t i = 0; i < Size; i++)
	{
		// 경로가 있다면 vecFullPath에 복사한다.
		if (Path)
	...
	if (Result)
	{
		for (size_t i = 0; i < Size; i++)
		{
			// 프레임 텍스처는 파일 하나하나의 이름들을 저장해둔다.(세이브 로드 용)
...
bool CTexture::LoadTextureFullPath(const std::vector<std::wstring>& vecFullPath)
{
	...
	for (size_t i = 0; i < Size; i++)
	{
		...
		// 비트맵을 로딩한다.
		// string이나 wstring 클래스의 c_str() 함수는 문자열 포인터를 얻어온다.
		HBITMAP hBmp = (HBITMAP)LoadImage(CGameManager::GetInst()->GetWindowInstance(),
