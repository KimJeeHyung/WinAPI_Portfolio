...
#include "resource.h"       // IconID를 사용하기 위해서 포함시킨다.
#include "Timer.h"
...
int CGameManager::Run()
{
    ...
    // 단, 메세지 큐가 비어있을 경우 메세지가 들어올 때까지 이 함수를 빠져나올 수 없다.
    // 이렇게 멈춰서 대기하고 있는 것을 블로킹 모드라고 한다.
    // 큐가 비어서 멈춰있는 시간을 윈도우의 데드타임이라고 부른다.
    while (m_Loop)
    {
        // PeekMessage : 이 함수도 메세지 큐에서 메세지를 꺼내오는 함수이다.
        // 단, 이 함수는 메세지 큐가 비어있을 경우 false를 반환하며
        // 바로 빠져나오게 된다.
        if (PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE))
        {
            // 메세지큐에서 꺼내온 메세지를 TranslateMessage 함수로 넘겨주면
            // 문자 키인지 F1, 방향키같은 키인지를 판단해준다.
            // 이러한 키들은 WM_KEYDOWN으로 메세지가 인식이 되고
            // 문자 키는 WM_CHAR로 인식이 된다.
            // 키를 누르면 문자키의 경우 WM_CHAR도 만들어져야 하기 때문에
            // WM_KEYDOWN이 일어나면 문자키의 경우 WM_CHAR메세지를 추가로 만들어서
            // 메세지 큐에 넣어준다.
            TranslateMessage(&msg);

            // DispatchMessage 함수는 메세지큐에서 꺼내온 메세지를 메시지 처리 함수에
            // 보내준다. WndProc로 보내주는 것이다.
            DispatchMessage(&msg);
        }
        // 메세지 큐가 비어있을 경우 동작된다. 즉, 윈도우의 데드타임 시
        // 이 else 구문이 동작되기 때문에 여기에 실제 게임로직을 작성하면 된다.
        else
        {
            Logic();
        }
...
void CGameManager::Logic()
{
    ...
    // 델타타임에 TimeScale을 곱해 전체 게임의 속도를 조절한다.(0이면 게임이 멈춤)
...
{
    // 화면을 한번 갈아엎기 위해 임시로 화면 크기만큼 사각형을 그려준다.
    // 사각형의 테두리가 보이지 않도록 1씩 크게 잡아준다.
    ...
    // 9번인자 : 그리는 방법 지정(SRCCOPY는 소스(6번인자)를 복사하겠다는 의미)
    ...
    //// TextOut : 현재 프로젝트의 설정이 멀티바이트냐 유니코드냐에 따라 위의 두 함수 중
    //// 하나가 결정된다.
    //TextOut(m_hDC, 50, 50, TEXT("텍스트 출력"), lstrlen(TEXT("텍스트 출력")));
    //TextOut(m_hDC, 500, 50, TEXT("또 출력"), lstrlen(TEXT("또 출력")));

    //Rectangle(m_hDC, 100, 100, 200, 200);   // 속이 채워져있는 사각형(빈 사각형은 FrameRect)
    //Ellipse(m_hDC, 200, 100, 300, 200);     // 좌표로 만든 사각형에 내접하는 원 출력

    ////for (int i = 0; i < 100; i++)
    ////{
    ////    SetPixel(m_hDC, 350 + i, 100, RGB(255, 0, 0));  // 점 출력
    ////}
    ...
    //// 선을 새로 그리려면 MoveToEx를 다시 써야한다.
    //MoveToEx(m_hDC, 450, 300, nullptr);
    //LineTo(m_hDC, 500, 300);

    // 델타타임으로 사각형 이동
    /*static float Top = 100.0f;
    static float Bottom = 200.0f;
    ...
    // 사각형을 화면 안에서 왔다갔다 움직이게 하기(세로 720 기준)
    /*m_TestRc.top += m_Dir;
...
void CGameManager::Register()
{
    ...
    // 윈도우의 클라이언트 영역의 가로 또는 세로 크기가 변할 때 화면을 다시 그려준다.
    wcex.style = CS_HREDRAW | CS_VREDRAW;

    // 메세지큐에서 꺼내온 메세지를 인자로 전달하며 호출할 함수의 주소를 등록한다.
    ...
    // TEXT매크로는 프로젝트 설정이 유니코드로 되어있을 경우 유니코드 문자열로 만들어지고
...
bool CGameManager::Create()
{
   ...
   // 3번 인자는 이 윈도우 창이 어떻게 생성될지를 지정하는 옵션이다.
   // 4번, 5번 인자는 이 윈도우 창이 생성될 화면에서의 위치를 지정한다.
   // 픽셀로 지정한다. 예를 들어 1920x1080 해상도라면 거기에서 원하는 값을 넣어주면
   // 해당 위치에 나오게 된다.
   // 4번은 가로좌표, 5번은 세로좌표로 사용이 된다.
   // 6번, 7번 인자는 윈도우 창의 가로, 세로의 크기를 지정한다.
   // 픽셀단위로 지정을 해준다.
   ...
   // 만약 생성이 제대로 안 되었다면 0을 반환한다.
    ...
    // 윈도우 창을 보여준다. 1번 인자에 들어간 핸들의 윈도우 창을 보여줄지 말지를
    // 결정해준다.
