...
private:
	...
	CSharedPtr<CGameObject> m_Target;	// 카메라가 따라다닐 타겟(반드시 지정할 필요는 없음)
	CSharedPtr<CGameObject> m_Target2;	// 카메라가 따라다닐 타겟(반드시 지정할 필요는 없음)
	Vector2 m_TargetPos;				// 카메라가 따라다닐 타겟(오브젝트가 아닌 위치만)
	Vector2 m_TargetOffset;				// 타겟으로부터 얼마나 떨어져 있을지
	Vector2 m_TargetPivot;				// 타겟이 화면에서 스크롤링될 때의 비율
