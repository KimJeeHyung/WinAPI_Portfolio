...
CInput::~CInput()
{
	// 코드 블록을 쓴 이유는 비슷한 기능을 반복할 때 
	// 같은 이름의 변수를 다시 사용할 수 있기 때문
	// (블록 내에서 만든 변수는 블록이 끝나면 제거됨)
	{
...

	{
		...
		for (; iter != iterEnd; ++iter)
		{
			for (int i = 0; i < (int)Input_Type::End; i++)
			{
				...
				{
					// vecFunction에 넣어준 함수들은 동적할당으로 생성했기 때문에 소멸자에서
					// 제거해줘야 한다.
...
{
	// 마우스 위치에 카메라 위치를 더해 실제 월드 내의 마우스 좌표를 구한다,
...
{
	POINT ptMouse;	// 마우스 좌표를 받는 구조체

	// 마우스의 스크린 좌표(윈도우 창이 아닌 모니터 전체에 대한 좌표)가 나온다.
	...
	// 마우스 왼쪽 버튼 클릭 상태를 실시간으로 처리한다.
	// 마우스 왼쪽 버튼을 눌렀을 때
	if (GetAsyncKeyState(VK_LBUTTON) & 0x8000)
	{
		// Down과 Push가 모두 false라면 이제 막 누른 것이다.
	...
	// 1프레임 후 Up도 false로 변경
	else if (m_MouseLUp)
		m_MouseLUp = false;

	// 마우스 오브젝트의 위치를 화면 내에서의 마우스 좌표로 정해준다.
...
void CInput::UpdateKeyState(float DeltaTime)
{
	...
	for (; iter != iterEnd; ++iter)
	{
		...
		if (KeyPush)
		{
			// Down과 Push가 모두 false라면 이 키를 지금 누른 것이다.
			// 그러므로 둘 다 true로 변경한다.
			...
			// Down과 Push 둘 중 하나라도 true라면 Down은 false가 되어야 한다.
...
void CInput::UpdateBindKey(float DeltaTime)
{
	...
	{
		// 키를 눌렀을 때(Down)
		// (조합키와 같이 누르는 키의 경우 그 조합키도 같이 눌렀는지 확인)
