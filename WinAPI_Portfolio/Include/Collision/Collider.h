...
protected:
    ...
    class CGameObject*  m_Owner;        // 자신을 갖고 있는 오브젝트
    ECollider_Type      m_ColliderType; // 충돌체 타입(모양)
    Vector2             m_Offset;       // Owner로부터 얼마만큼 떨어져 있을지.
    CollisionProfile*   m_Profile;      // 충돌 프로파일
    Vector2             m_HitPoint;
    float               m_Bottom;       // Y소팅 시 기준이 될 바닥 위치
    ...
    /*
    현재 프레임에 두 물체가 충돌되고 다음 프레임에도 두 물체가 충돌이 되었다면
    계속 충돌이 되고 있는 상태로 판단해야 한다.
    ...
    */
    ...
    bool m_MouseCollison;   // 마우스는 충돌체가 아니므로 충돌체 리스트에 넣지 않는다.

public:
    void SetMouseCollision(bool Collision)
    {
        m_MouseCollison = Collision;
    }
